...
async fn test_deposit_withdraw_local() -> Result<(), ClientError> {
    ...
    println!("\nDeploying fungible faucet A");
    let faucet_a = create_basic_faucet(&mut client, keystore.clone()).await?;
    println!(
        "Faucet A account ID: {:?}",
        faucet_a.id().to_bech32(NetworkId::Testnet)
    );

    println!("\nDeploying a fungible faucet B");
    let faucet_b = create_basic_faucet(&mut client, keystore.clone()).await?;
    println!(
        "Faucet B account ID: {:?}",
        faucet_b.id().to_bech32(NetworkId::Testnet)
    );
    ...
    println!("\n[STEP 3] Mint tokens for Alice");
    let amount: u64 = 100;
    let mint_amount_a = FungibleAsset::new(faucet_a.id(), amount).unwrap();
    ...
    let tx_exec_a = client.new_transaction(faucet_a.id(), tx_request).await?;
    ...
    let amount: u64 = 100;
    let mint_amount_b = FungibleAsset::new(faucet_b.id(), amount).unwrap();
    let tx_request = TransactionRequestBuilder::new()
        .build_mint_fungible_asset(
            mint_amount_b,
            alice_account_id,
    ...
    let tx_exec_b = client.new_transaction(faucet_b.id(), tx_request).await?;
    ...
    wait_for_note(
        ...
        &p2id_note_a,
        tx_exec_b.executed_transaction().id(),
    )
    .await?;
    ...
    let vault = NoteAssets::new(vec![mint_amount_a.into()])?;
    let deposit_note_a = Note::new(vault, metadata, recipient);
    println!("deposit note hash: {:?}", deposit_note_a.id().to_hex());
    ...
    let vault = NoteAssets::new(vec![mint_amount_b.into()])?;
    let deposit_note_b = Note::new(vault, metadata, recipient);
    println!("deposit note hash: {:?}", deposit_note_b.id().to_hex());
    ...
    let output_amount = FungibleAsset::new(faucet_b.id(), 0).unwrap().into();

    // Create a P2ID note with the same asset amount, targeted to Alice
    let output_p2id_note = create_p2id_note(
        deposit_contract.id(), // sender (the contract)
        alice_account_id,      // target (Alice)
        vec![output_amount],   // same asset that was deposited
        NoteType::Private,
        Felt::new(0),
        client.rng(),
    )
    ...
    println!(
        "output_p2id_note P2ID note hash: {:?}",
        output_p2id_note.id().to_hex()
    );
    println!("Withdraw note assets: {:?}", output_p2id_note.assets());

    // -------------------------------------------------------------------------
    // STEP 7: Create swap amm note
    // -------------------------------------------------------------------------
    println!("\n[STEP 7] Create swap amm note with assets");
    ...
    let p2id_withdraw_recipient: Word = output_p2id_note.recipient().digest().into();
    ...
    let note_inputs = NoteInputs::new(vec![
        ...
        output_p2id_note.metadata().execution_hint().into(),
        output_p2id_note.metadata().note_type().into(),
        Felt::new(0),
        output_p2id_note.metadata().tag().into(),
    ])
    .unwrap();
    ...
    // -------------------------------------------------------------------------
    // STEP 8: Consume the swap amm note
    // -------------------------------------------------------------------------
    println!("\n[STEP 8] Consume the swap amm note");
    ...
    let consume_deposit_request = TransactionRequestBuilder::new()
        .unauthenticated_input_notes([(output_p2id_note.clone(), None)])
        .build()
        .unwrap();
    let tx_result = client
